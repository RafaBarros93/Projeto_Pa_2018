                ...
                <table class="table table-striped" id="lista_registros">
                    ...
                    <tbody id="lista_registros_corpo">
                        <?php  
                            ...
                            $ch = curl_init();
                            curl_setopt($ch, CURLOPT_URL, BASE_URL_SERVICO."/advogado");
                            ...
                            for($i=0;$i<count($dados); $i++){
                                echo '<tr id="linha'.$dados[$i]['CodigoAdvogado'].'">';
                                    echo '<td>'.$dados[$i]['Descricao'].'</td>';
                                    echo '<td>'.$dados[$i]['CodigoOAB'].'</td>';
                                    echo '<td>';
                                        echo '<button class="btn btn-primary btn-xs" onclick="editar('.$dados[$i]['CodigoAdvogado'].')">Editar</button>&nbsp;';
                                        echo '<button class="btn btn-danger btn-xs" onclick="excluir('.$dados[$i]['CodigoAdvogado'].')">Excluir</button>';
                                    echo '</td>';
                                echo '</tr>';
                            }
                        ?>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal-titulo">Advogado</h4>
                </div>
                <div class="modal-body">
                    <form class="formulario form-horizontal" name="form">
                        ...
                        <div class="form-group form-group-sm">
                            <label class="col-sm-3 control-label" for="txtCodigoOAB">Registro OAB</label>
                            <div class="col-sm-9">
                                <input type="number" id="txtCodigoOAB" class="form-control" placeholder="Insira o registro OAB">
                            </div>
                        </div>
                    </form>
                </div>
...
<script src="js/formularios/advogado.js"></script>
