                ...
                <table class="table table-striped" id="lista_registros">
                    ...
                    <tbody id="lista_registros_corpo">
                        <?php  
                            ...
                            $ch = curl_init();
                            curl_setopt($ch, CURLOPT_URL, BASE_URL_SERVICO."/menu");
                            ...
                            for($i=0;$i<count($dados); $i++){
                                echo '<tr id="linha'.$dados[$i]['CodigoMenu'].'">';
                                    echo '<td>'.$dados[$i]['Descricao'].'</td>';
                                    echo '<td>'.$dados[$i]['Ativo'].'</td>';
                                    echo '<td>';
                                        echo '<button class="btn btn-primary btn-xs" onclick="editar('.$dados[$i]['CodigoMenu'].')">Editar</button>&nbsp;';
                                        echo '<button class="btn btn-danger btn-xs" onclick="excluir('.$dados[$i]['CodigoMenu'].')">Excluir</button>';
                                    echo '</td>';
                                echo '</tr>';
                            }
                        ?>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal-titulo">Menu</h4>
                </div>
...
<script src="js/formularios/menu.js"></script>
