...
function salvar(){
    ...
    if (descricao === ''){
        ...
        tipoAcao.CodigoTipoAcao = codigo;
        tipoAcao.Descricao = descricao;
    
        var dados = JSON.stringify(tipoAcao);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/tipoAcao", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/tipoAcao");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/tipoAcao/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}