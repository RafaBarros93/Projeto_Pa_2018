...
function salvar(){
    ...
    if (descricao === ''){
        ...
        empreiteiro.CodigoEmpreiteiro = codigo;
        empreiteiro.Descricao = descricao;
    
        var dados = JSON.stringify(empreiteiro);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/empreiteiro", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/empreiteiro");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/empreiteiro/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}