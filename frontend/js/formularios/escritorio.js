...
function salvar(){
    ...
    if (descricao === ''){
        ...
        escritorio.CodigoEscritorio = codigo;
        escritorio.Descricao = descricao;
    
        var dados = JSON.stringify(escritorio);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/escritorio", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/escritorio");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/escritorio/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}