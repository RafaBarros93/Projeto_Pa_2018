...
function salvar(){
    ...
    if (descricao === ''){
        ...
        funcaoAlegada.CodigoFuncaoAlegada = codigo;
        funcaoAlegada.Descricao = descricao;
    
        var dados = JSON.stringify(funcaoAlegada);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/funcaoAlegada", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/funcaoAlegada");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/funcaoAlegada/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}