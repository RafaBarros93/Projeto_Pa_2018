...
function salvar(){
    ...
    if (descricao === ''){
        ...
        esfera.CodigoEsfera = codigo;
        esfera.Descricao = descricao;
    
        var dados = JSON.stringify(esfera);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/esfera", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/esfera");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/esfera/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}