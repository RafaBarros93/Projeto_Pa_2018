...
function salvar(){
    ...
    if (descricao === ''){
        ...
        origem.CodigoOrigem = codigo;
        origem.Descricao = descricao;
    
        var dados = JSON.stringify(origem);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/origem", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/origem");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/origem/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}