...
function salvar(){
    ...
    if (descricao === ''){
        ...
        varaLocal.CodigoVaraLocal = codigo;
        varaLocal.Descricao = descricao;
    
        var dados = JSON.stringify(varaLocal);
...
//funcao para gravar um registro no bd
function gravar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("POST", BASE_URL_SERVICO + "/varaLocal", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/varaLocal");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/varaLocal/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}