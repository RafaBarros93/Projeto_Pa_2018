...
function salvarRegistro(){
    ...
    let keyUser = document.getElementById('txtKeyUser').value;
    let atividade = document.getElementById('txtAtividade').value;
    let problema = document.getElementById('txtProblema').value;
    let solucaoProposta = document.getElementById('txtSolucaoProposta').value;
    let status = document.getElementById('txtStatus').value;

    let registro = {};
    if(codigo != ''){
        registro.CodigoOportunidadeProblema = codigo;
    } else {
    ...
    registro.KeyUser = keyUser;
    registro.Atividade = atividade;
    registro.Problema = problema;
    registro.SolucaoProposta = solucaoProposta;
...
function editarRegistro(indice){
    controlaPanel(true);
    document.getElementById('txtCodigo').value = registros[indice].CodigoOportunidadeProblema;
    document.getElementById('txtID').value = registros[indice].ID;
    document.getElementById('selNomeProcesso').value = registros[indice].CodigoEstruturaProcesso;
    document.getElementById('txtKeyUser').value = registros[indice].KeyUser;
    document.getElementById('txtAtividade').value = registros[indice].Atividade;
    document.getElementById('txtProblema').value = registros[indice].Problema;
    document.getElementById('txtSolucaoProposta').value = registros[indice].SolucaoProposta;
    document.getElementById('txtStatus').value = registros[indice].Status;
}
...
function excluirRegistro(indice){
    ...
    let codigo = registros[indice].CodigoOportunidadeProblema;
    deletarBD(codigo);
...
function preencheTable(){
    ...
    for(let i in registros){
        ...
        <tr>
            ...
            <td>${registros[i].KeyUser}</td>
            <td>${registros[i].Atividade}</td>
...
function gravarBD(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/oportunidadeProblema", false);//realiza uma chamada sincrona para receber o id gerado
...
function deletarBD(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/oportunidadeProblema/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}

//funcao para carregar os registros do bd
function carregaOportunidadeProblema(){
    let xhr = new XMLHttpRequest();
    xhr.open('GET', BASE_URL_SERVICO + '/oportunidadeProblema', false);
    xhr.onload = function(){
        if(xhr.status == 200){
