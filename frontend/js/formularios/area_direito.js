...
function salvar(){
    ...
    if (descricao === ''){
        ...
        areaDireito.CodigoAreaDireito = codigo;
        areaDireito.Descricao = descricao;
    
        var dados = JSON.stringify(areaDireito);
...
function gravar(dados){
    ...
    xhr.open("POST", BASE_URL_SERVICO + "/areaDireito", false);//realiza uma chamada sincrona para receber o id gerado
...
function alterar(dados){
    var xhr = new XMLHttpRequest();
    xhr.open("PUT", BASE_URL_SERVICO + "/areaDireito");
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(dados);
}
...
function deletar(id){
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", BASE_URL_SERVICO + "/areaDireito/"+ id);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send();
}